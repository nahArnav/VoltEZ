...
def _reliability_frame(role: str, world: str, seed: int, rows: int = 180) -> pd.DataFrame:
    rng = np.random.default_rng(seed)
    eta = rng.choice([15.0, 30.0, 60.0, 120.0], size=rows)
    status = rng.choice(
        ["reliable", "occupied", "faulted"],
        size=rows,
        p=[0.72, 0.23, 0.05],
    )
    ...
    unreliable_probability = 1 / (1 + np.exp(-logit))
    unreliable = rng.random(rows) < unreliable_probability
    unreliable[0] = False
    unreliable[1] = True
    return pd.DataFrame(
        {
            ...
            "label": np.where(unreliable, "unreliable", "reliable"),
            "label_confidence": np.where(unreliable, 0.98, 0.99),
            "eta_minutes": eta,
...
def _suite(tmp_path: Path) -> Path:
    ...
    for role, world, seed in definitions:
        ...
        _reliability_frame(role, world, seed).to_parquet(path, index=False)
        entries.append(
            {
                "evaluation_role": role,
                "run_id": world,
                "tables": {"reliability_features_labeled": str(path)},
            }
        )
    ...
    write_manifest(
        {
            "models": {"reliability": {"status": "ready", "failures": []}},
        },
        readiness_path,
    )
...
def test_feature_contract_excludes_identity_and_label_columns() -> None:
    frame = _reliability_frame("train", "world", 10)
...
def test_thresholds_create_an_explicit_unknown_band() -> None:
    ...
    settings = ReliabilityTrainingSettings(minimum_threshold_rows=5)
...
def test_end_to_end_training_uses_train_validation_and_stress_only(
    ...
    assert artifact["positive_class"] == "unreliable"
