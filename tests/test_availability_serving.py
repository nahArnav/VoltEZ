...
def _predictor(
...
) -> tuple[AvailabilityPredictor, AvailabilityFeatureRequest]:
    ...
    joblib.dump(
        {
            ...
            "positive_class": "unavailable",
            "negative_class": "available",
            "thresholds": {
                "available_max_probability_unavailable": 0.1,
                "unavailable_min_probability_unavailable": 0.6,
            },
    ...
    rules = {
        ...
        "latest_status": {
            "kind": "categorical",
            "allow_missing": False,
            "allowed_values": ["available", "occupied", "faulted", "unknown"],
        },
    }
    contract = {
    ...
    predictor = AvailabilityPredictor.from_artifact(artifact_dir, contract_path)
...
def test_calibrated_probability_and_three_state_decision(tmp_path: Path) -> None:
    available_predictor, request = _predictor(tmp_path / "available", 0.05)
    available = available_predictor.predict(request)
    assert available.decision == "available"
    assert available.quality == "in_domain"
    assert available.probability_unavailable == pytest.approx(0.05)
    ...
    assert unknown.probability_unavailable == pytest.approx(0.3)
...
def test_unknown_status_forces_abstention_without_probability(tmp_path: Path) -> None:
    ...
    assert response.probability_unavailable is None
...
def test_schema_and_timestamp_disagreement_are_rejected(tmp_path: Path) -> None:
    ...
    with pytest.raises(AvailabilityInputError, match="timestamps"):
        predictor.predict(bad_time)
    with pytest.raises(AvailabilityInputError, match="schema mismatch"):
