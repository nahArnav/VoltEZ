...
// ─────────────────────────────────────────────────────────────────────────────
// 3D HOLOGRAPHIC VECTOR & MESH CAR PAINTER
// ─────────────────────────────────────────────────────────────────────────────
class _CarHologramPainter extends CustomPainter {
  ...
  void _drawHolographicPedestal(
    ...
    for (var a = 0.0; a < 2 * math.pi; a += math.pi / 16) {
      ...
      canvas.drawLine(
        ...
        Offset(
          baseCenter.dx + radX * (1 - len / 142),
          baseCenter.dy + radY * (1 - len / 34),
        ),
        tickPaint,
      );
    }
  ...
  void _draw3DSportsCarMesh(
    ...
    // ── WINDSHIELD / CABIN ──
    final pABeltFront = Offset(ox - 46 * s, oy - 10 * s);
    final pAPillarTop = Offset(ox - 28 * s, oy - 46 * s);
    final pRoofFront = Offset(ox + 2 * s, oy - 52 * s);
    final pRoofPeak = Offset(ox + 48 * s, oy - 50 * s);
    final pRoofRear = Offset(ox + 88 * s, oy - 42 * s);
    final pCPillarTop = Offset(ox + 112 * s, oy - 26 * s);
    ...
    final frontWindow = Path()
      ...
      ..lineTo(
        pRearGlassBotFront.dx,
        pRearGlassBotFront.dy,
      )
      ..lineTo(
        pBeltCPillar.dx,
        pBeltCPillar.dy,
    ...
    canvas.drawPath(
      Path()
        ..moveTo(
          pLowerCreaseFront.dx,
          pLowerCreaseFront.dy,
        )
        ..quadraticBezierTo(
          pLowerCreaseMid.dx,
          pLowerCreaseMid.dy,
    ...
    // Vertical door panel grid lines
    for (var i = 1; i <= 3; i++) {
      final frac = i / 4.0;

      final top = Offset(
        pBeltFront.dx +
            (pBeltCPillar.dx - pBeltFront.dx) * frac,
        pBeltFront.dy +
            (pBeltCPillar.dy - pBeltFront.dy) * frac,
      );

      final bot = Offset(
        pSillFront.dx +
            (pSillRear.dx - pSillFront.dx) * frac,
        pSillFront.dy +
            (pSillRear.dy - pSillFront.dy) * frac,
      );
    ...
    // 10. CHARGING PORT
    // ════════════════════════════════════════════════════════
    final chargePort = Offset(
      ox - 52 * s,
      oy + 14 * s,
    );

    final portPulse = Paint()
      ..color = holoMint.withValues(
        alpha: 0.65 + 0.35 * math.sin(t * math.pi * 4),
      )
      ..style = PaintingStyle.fill;
  ...
  void _draw3DWheel(
    ...
    for (var i = 0; i < spokes; i++) {
      ...
      final sx =
          center.dx + (radiusX * 0.68) * math.cos(a);

      final sy =
          center.dy + (radiusY * 0.68) * math.sin(a);
  ...
  void _drawHoloParticles(
    ...
    for (var i = 0; i < 20; i++) {
      ...
      final alpha =
          math.sin(phase * math.pi) * 0.60;

      particlePaint.color =
          (i % 2 == 0 ? holoCyan : holoMint)
              .withValues(
  ...
  @override
  bool shouldRepaint(
    covariant _CarHologramPainter oldDelegate,
  ) {
    return true;
  }
}