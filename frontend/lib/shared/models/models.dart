...
class Vehicle {
  ...
  String get primaryConnector =>
      connectorTypes.isNotEmpty ? connectorTypes.first : 'Unknown';
...
class Charger {
  ...
  final String name;
  final String?
  address; // Not in backend — may be computed from lat/lng or omitted
  ...
  /// Highest power port.
  double get maxPortPower => ports.isEmpty
      ? powerKw
      : ports.map((p) => p.maxPowerKw).reduce((a, b) => a > b ? a : b);
  ...
  List<ConnectorType> get connectors => ports.isNotEmpty
  ...
  factory Charger.fromJson(Map<String, dynamic> json) {
    ...
    final legacyConnectors =
        (json['connectors'] as List<dynamic>?)?.map((e) {
          final s = e as String;
          switch (s) {
            case 'ccs2':
              return ConnectorType.ccs2;
    ...
    switch (statusStr) {
      case 'available':
        legacyStatus = ChargerStatus.available;
        break;
      case 'unavailable':
        legacyStatus = ChargerStatus.busy;
        break;
      case 'busy':
        legacyStatus = ChargerStatus.busy;
        break;
      case 'offline':
        legacyStatus = ChargerStatus.offline;
        break;
      case 'maintenance':
        legacyStatus = ChargerStatus.maintenance;
        break;
      case 'active':
        legacyStatus = ChargerStatus.available;
        break;
    ...
    return Charger(
      ...
      name: json['name'] as String,
      address: (json['address'] ?? json['address_text']) as String?,
  ...
  Map<String, dynamic> toJson() => {
    ...
    'ports': ports
        .map(
          (p) => {
            'id': p.id,
            'charger_id': p.chargerId,
            'connector_type': p.connectorType,
            'max_power_kw': p.maxPowerKw,
            'status': p.status,
          },
        )
        .toList(),
...
class ChargingSession {
  ...
  factory ChargingSession.fromJson(Map<String, dynamic> json) =>
      ChargingSession(
        ...
        checkInAt: json['reserved_at'] != null
            ? DateTime.tryParse(json['reserved_at'] as String)
            : null,
        startAt: json['started_at'] != null
            ? DateTime.tryParse(json['started_at'] as String)
            : null,
        endAt: json['ended_at'] != null
...
class Review {
  ...
  factory Review.fromJson(Map<String, dynamic> json) => Review(
    ...
    issueFlags: (json['issue_flags'] as List<dynamic>?)
        ?.map((e) => e as String)
        .toList(),
    createdAt: json['created_at'] != null
        ? DateTime.tryParse(json['created_at'] as String)
